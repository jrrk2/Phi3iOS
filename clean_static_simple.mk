...
LIBS = -L$(BUILD_DIR) -lonnxruntime other.a \
       -Wl,-rpath,$(BUILD_DIR) \
...
../onnxruntime-genai/build/macOS/RelWithDebInfo/CMakeFiles/onnxruntime-genai.dir/src/models/windowed_kv_cache.cpp
...
simple: check-requirements $(SIMPLE_TARGET)
...
# Build the simple demo (minimal API usage)
$(SIMPLE_TARGET): check-requirements
	@echo "🔨 Building simple C API demo..."
	@echo "📦 Using minimal C API calls only"
	$(CXX) $(CXXFLAGS) $(INCLUDES) -o $(SIMPLE_TARGET) $(SIMPLE_DEMO) \
		$(ALL_SOURCES) $(MISSING_FUNCS) $(WORKING_TOKENIZER) $(LIBS)
	@if [ -f $(SIMPLE_TARGET) ]; then \
		echo "✅ Simple demo build successful!"; \
		echo "🍎 Executable: $(SIMPLE_TARGET)"; \
		echo "🚀 Try: ./$(SIMPLE_TARGET) your_model.onnx"; \
		echo "💡 This uses only basic C API calls - most likely to work!"; \
	else \
		echo "❌ Simple demo build failed"; \
	fi
