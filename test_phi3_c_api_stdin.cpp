...
int main() {
    ...
    // Path to your Phi-3 model (update this path)
    ...
    try {
        ...
        if (user_input.empty()) {
            ...
            std::cout << "🔄 Using default question: '" << user_input << "'\n";
        }
        ...
        std::cout.flush(); // Ensure prompt is shown before generation
        ...
        while (!OgaGenerator_IsDone(generator) && token_count_generated < max_tokens) {
            ...
            if (token_count > 0) {
                ...
                if (OgaTokenizerStreamDecode(tokenizer_stream, new_token, &token_text) == nullptr) {
                    std::cout << token_text;
                    std::cout.flush(); // Show tokens as they're generated
                    response += token_text;
